    ...
    >
      <Container>
        ...
        <ul className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 sm:gap-4 list-none p-0 m-0">
          {assets.screenGrabsAtNext.map((img) => (
            <li key={img.id} className="relative aspect-video rounded-lg overflow-hidden bg-[var(--tone-surface)] border border-[var(--tone-border)]">
              <Image
                src={img.src}
